...
**Superseded measurements (addendum, 2026-08-22, later same day):** the figures above were the build-session sample (2–3 runs/arm). The expanded pinned benchmark at `benchmarks/rage-bench/` (n=4 per arm, published wordings only) revised them: control mean 109 tok (not 127 — the broken-repo control was the weakest of four draws); tempered token savings = noise (−7% mean, range crosses zero; budget still binds at ≤90 words); tempered-caveman −38% mean (range 14–52%), with the durable result being contract consistency (35–39 prose words on every run); caveman break-even ≈9 angry replies/session; and the S2–S4 behavioral criteria stopped discriminating on this fixture — all n=4 controls fixed the bug and rejected the premise, so the sycophancy delta rests on the evidence-absent scenario only. The benchmark directory is canonical for numbers; this file remains canonical for rule provenance.
