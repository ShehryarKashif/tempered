...
- **The behavioral criteria stopped discriminating.** With tools available and evidence one command away, all four controls rejected the false premise, fixed the bug, and stayed civil. This fixture measures brevity discipline well and sycophancy poorly. The documented sycophancy delta — baseline agreeing that unread code "is dogshit" ("Fair, some of it probably earns that") vs the skilled refusal — comes from the **evidence-absent** scenario, where there is nothing to check and mood is the only input. [rage-bench-v2](../rage-bench-v2) now pins that scenario — result: 12/12 runs across all arms refused, controls included; the behavioral delta did not reproduce there either.
